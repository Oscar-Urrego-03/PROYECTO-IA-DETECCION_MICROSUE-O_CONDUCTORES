...
class ReportVisualizer:
    ...
    def draw_report_text(self, sketch: np.ndarray, text: str, position: Tuple[int, int], color: Tuple[int, int, int]):
        cv2.putText(sketch, text, position, cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 1)
    ...
    def draw_warnings_general(self, sketch: np.ndarray, feature: str):
        ...
        translations = {
            'micro_sleep': 'microsueno',
            'pitch': 'inclinacion'
        }
        eye_translations = {
            'eye_rub_first_hand': 'frotado de ojos primera mano',
            'eye_rub_second_hand': 'frotado de ojos segunda mano',
            'flicker': 'parpadeo',
            'yawn': 'bostezo'
        }
        ...
        if feature == 'micro_sleep' or feature == 'pitch':
            if feature == 'micro_sleep':
                self.draw_report_text(sketch,
                                      f"evaluando: {translations.get(feature, feature)}: esperando ...",
                                      position, color)
            ...
            translation = eye_translations.get(feature, feature)
            self.draw_report_text(sketch,
                                  f"contando: {translation}: {5 - elapsed_time} esperando...",
                                  position, color)
    ...
    def draw_warnings_report(self, sketch: np.ndarray, feature: str):
        ...
        translation = translations.get(feature, feature)
        self.draw_report_text(sketch, f"{translation} {feature}: {feature_count}", position, color)

        if feature == 'flicker':
            text = f"{feature.replace('_', ' ')} {feature}: {feature_count}"
            text_size, _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 1.2, 1)
